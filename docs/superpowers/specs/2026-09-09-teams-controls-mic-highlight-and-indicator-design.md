...
```
   ●  Not in call
   ●  Pre-join       🎤 Live      (mic chip: teal #0d9488 / red #dc2626)
   ●  In call        🔇 Muted
```
...
- **Mic chip** (shown for `pre-join` and `in-call`): `micState === "live"` → 🎤 "Live"
  (teal); `"muted"` → 🔇 "Muted" (red); `null` → chip hidden.

Colors match the on-page ring. The indicator uses the existing popup styling idiom
(inline `<style>`, emoji glyphs like the current ⚙️) and includes an `aria-live="polite"`
